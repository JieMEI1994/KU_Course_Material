...
double  GAMMA = 1.0;
double  TEMPERATURE = 200;  // 温度：いろいろと変えてみよう
...
int main( )
{
   ...
   double entropy;
   FILE *fentropy;
   fentropy = fopen("entropy.dat", "w");   //エントロピー出力
   ...
   for (ix=1;ix<NX_CELL-1;ix++) {         // 確率密度の更新と再規格化，ただし両端はゼロのまま
      ...
      sum*=dx;
      entropy = 0.0;
		for (ix = 1; ix<NX_CELL - 1; ix++) {         //再規格化後にエントロピーを求める
			prob[ix] /= sum;
			if ((prob[ix] > 0.0) && (prob_eq[ix] > 0.0)) {
				entropy += -prob[ix] * log(prob[ix] / prob_eq[ix]);
			}
		}

		entropy *= dx;
		printf("%7d %8.6f %10.5f\n", nstep, DT*nstep, entropy);
		fprintf(fentropy, "%7d %8.6f %10.5f\n", nstep, DT*nstep, entropy);
